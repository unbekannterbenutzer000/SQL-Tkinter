...
class BD(Main):
    ...
    def createtb(self):
        ...
        with self.connection1.cursor() as cursor:
            self.sql2 = f"CREATE TABLE IF NOT EXISTS {page1.TB_name.get()} (ID int, Стэк MEDIUMTEXT, Удаленные TEXT);"
...
class win2(Main):
    ...
    def to_sql(self):
        ...
        stek1 = queue.LifoQueue(maxsize=500)
        ...
        deleted = ''
        for i in range(100):
            deleted += str(stek1.get())
            deleted += ' '


        msg.showinfo('Успех', 'Данные внесены')
        ...
        Main.connection1(self)
        with self.connection1.cursor() as cursor:
            self.sql3 = f"INSERT INTO {page1.TB_name.get()} (ID, Стэк, Удаленные) VALUES (%s, %s, %s)"
            cursor.execute(self.sql3, (page2.string_id.get(), data, deleted))
            self.connection1.commit()
...
class win3(Main):
    ...
    def save(self):
        ...
        msg.showinfo('Успех', 'Таблица создана')
