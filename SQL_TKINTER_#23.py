...
class win2(Main):
    ...
    def start(self):
        ...
        # Поле 2
        text3 = tk.Label(window2, text="Введите Cписок")
    ...
    def to_sql(self):
        ...
        with self.connection1.cursor() as cursor:
            self.sql3 = f"INSERT INTO {page1.entry_widget_2.get()} (ID, Список, Индексы) VALUES (%s, %s, %s)"
            cursor.execute(self.sql3, (page2.entry_widget_3.get(), sps1, sps2))
...
class BD(Main):
    ...
    def createtb(self):
        Main.connection1(self)
        with self.connection1.cursor() as cursor:
            self.sql2 = f"CREATE TABLE IF NOT EXISTS {page1.entry_widget_2.get()} (ID int, Список varchar(2000), Индексы varchar(2000));"
