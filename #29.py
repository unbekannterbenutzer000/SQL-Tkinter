...
class BD(Main):
    ...
    def createtb(self):
        ...
        with self.connection1.cursor() as cursor:
            self.sql2 = f"CREATE TABLE IF NOT EXISTS {page1.TB_name.get()} (ID int, Стэк VARCHAR(10000));"
...
class win1:
    ...
    def start(self):
        ...
        btn_q = cutk.CTkButton(window, text="Выйти из программы", command=quit, fg_color="#DB3E39", hover_color="#821D1A")
...
class win2(Main):
    ...
    def start(self):
        ...
        btn_cls = cutk.CTkButton(window2, text="Закрыть окно", command=window2.destroy, fg_color="#DB3E39", hover_color="#821D1A")
...
class win3(Main):
    def start(self):
        ...
        # Кнопка закрытия окна
        btn15 = cutk.CTkButton(window3, text="Закрыть окно", command=window3.destroy, fg_color="#DB3E39", hover_color="#821D1A")
