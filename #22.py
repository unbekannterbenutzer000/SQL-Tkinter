...
class win2(Main):
    ...
    def start(self):
        ...
        # Поле 2
        text3 = tk.Label(window2, text="Введите градусы")
    ...
    def to_sql(self):
        degree = float(page2.entry_widget_4.get())
        radian = (degree * math.pi) / 180
        msg.showinfo("Ответ", str(radian))

        Main.connection1(self)
        with self.connection1.cursor() as cursor:
            self.sql = f"INSERT INTO {page1.entry_widget_2.get()} (ID, Градусы, Радианы) VALUES (%s, %s, %s)"
            cursor.execute(self.sql, (page2.entry_widget_3.get(), degree, radian))
            self.connection1.commit()
            self.df = pd.read_sql_query(f"SELECT * from {page1.entry_widget_2.get()}", self.connection1)
            msg.showinfo("Данные из таблицы MySQL", str(self.df))
...
class win3(Main):
    ...
    def save(self):
        Main.connection1(self)
        df1 = pd.read_sql_query(f"SELECT * FROM {page1.entry_widget_2.get()}", self.connection1)
        df1.to_excel(self.entry_widget_10.get(), sheet_name='Данные из MySQL', index=False)
...
class BD(Main):
    ...
    def createdb(self):
        ...
        with self.connection.cursor() as cursor:
            self.sql2 = f"CREATE DATABASE IF NOT EXISTS {page1.entry_widget_1.get()}"
            cursor.execute(self.sql2)
            self.connection.commit()
            self.df2 = pd.read_sql("SHOW DATABASES", self.connection)  # считывание информации из базы данных
    ...
    def createtb(self):
        Main.connection1(self)
        with self.connection1.cursor() as cursor:
            self.sql3 = f"CREATE TABLE IF NOT EXISTS {page1.entry_widget_2.get()} (ID int, Градусы float, Радианы float);"
            cursor.execute(self.sql3)
            self.connection1.commit()
            self.df3 = pd.read_sql("SHOW TABLES", self.connection1)  # считывание информации из базы данных
            msg.showinfo("Базы данных", str(self.df3))
            self.connection1.close()
        return
